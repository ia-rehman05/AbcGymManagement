...
namespace AbcGymManagement.ApiRequestHandler
{
    public class HttpTrainerHandler
    {
        ...
        public HttpTrainerHandler()
        {
            _trainer = new HttpClient();
        ...
        public async Task<bool> UpdateTrainerAsync(string fullUrl, TrainerCreateDto trainerDto)
        {
            var jsonContent = JsonConvert.SerializeObject(trainerDto);
            var content = new StringContent(jsonContent, Encoding.UTF8, "application/json");
        ...
        public async Task<List<TrainerCreateDto>> GetAllTrainersAsync(string fullUrl)
        {
            try
            {
                HttpResponseMessage response = await _trainer.GetAsync(fullUrl);

                if (response.IsSuccessStatusCode)
                {
                    string responseContent = await response.Content.ReadAsStringAsync();
                    var apiListResponse = JsonConvert.DeserializeObject<ApiListResponse<TrainerCreateDto>>(responseContent);
